...
const tokan = localStorage.getItem('token');
  const topGetPopular = async() => {
    try{
      const res = await axios.get(`${BASE_URL}/3/trending/movie/day?language=en-US`,{
        headers: {
          Authorization : `Bearer ${tokan}`
        }
      })
      console.log(res)
      setTopPopular(res.data.results)
  ...
  const showMovies = () => {
    setVisibleMovies(preVisibleMovies => preVisibleMovies + 5)
  }
      ...
      <aside className=" flex-none w-1/6 py-10 px-10 min-w-min border-l border-gray-300 dark:border-zinc-700 hidden lg:block">
        ...
        <div className="mt-10 text-start">
          ...
          <ul className="mt-4 text-gray-400 text-xs space-y-3">
            {/* Movie Item */}
            {
              topPopular.slice(0, visibleMovies).map((item, index) =>{
                return(
                  ...
                  <li className="flex space-x-3" key={index}>
                    ...
                    <div className="flex flex-col justify-between">
                      <div className="flex flex-col space-y-1">
                        <span className="text-gray-700 dark:text-white font-semibold">{item.title}</span>
